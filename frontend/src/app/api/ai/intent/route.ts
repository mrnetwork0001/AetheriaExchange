...
- PULSE is the category for short-dated (usually 24h) markets on X Layer's own public metrics - daily active wallets, OKB 24h trading volume (all venues, per CoinGecko), X Layer total DeFi TVL (per DefiLlama), X Layer total stablecoin circulating supply (per DefiLlama). Pulse questions must name the metric, the threshold, the measurement window, and the public data source; close them at the end of the measurement window. Say "trading volume" with its source, never "DEX volume" - the automated resolver settles against all-venue data.
- In every machine-resolvable question (PULSE, RWA, EQUITY) use the literal word "above" for the comparator - "closes above $X", "total TVL above $X". Never "exceeds", "more than", "at least", or "over": the automated resolver parses the exact phrase "above <threshold>", and a synonym strands the market in manual review.
